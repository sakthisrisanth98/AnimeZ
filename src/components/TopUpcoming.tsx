...
function AnimeGrid({
  ...
  <h3 className="text-xl md:text-2xl font-bold text-pink-400">{title}</h3>
  <a
    href="/view-more"
    className="text-sm text-white hover:underline ml-auto"
  >
    View more &gt;
  </a>
</div>
      ...
      <div className="flex overflow-x-auto space-x-4 md:space-x-8 scroll-smooth px-4 md:px-[30px] pb-2 hide-scrollbar">
        {animeList.map((anime, index) => (
          <div
            key={index}
            className="relative w-[120px] md:w-[180px] h-[180px] md:h-[260px] bg-zinc-800 rounded-md group flex-shrink-0"
          >
