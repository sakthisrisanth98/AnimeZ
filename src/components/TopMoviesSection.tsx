...
export default function TopMoviesSection() {
  ...
  return (
    <section className="px-4 md:px-6 pt-4 pb-0">
      {/* Title + View More */}
      <div className="flex items-center mb-4 md:mb-6">
        <h3 className="text-xl md:text-2xl font-bold text-pink-400">Top Movies</h3>
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
        ...
        <div className="flex-1">
          {[0, 6].map((start) => (
            <div key={start} className={`grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-6 ${start > 0 ? 'mt-6 lg:mt-10' : ''}`}>
              {topMovies.slice(start, start + 6).map((movie, i) => (
                <Link key={start + i} href={`/anime/${movie.slug ?? ''}`} className="group cursor-pointer relative">
        ...
        {/* Right: Top 5 Sidebar */}
        <div className="w-full lg:w-[285px] shrink-0">
          <h4 className="text-lg font-bold text-pink-400 mb-4 lg:mb-7">Top 5</h4>
          <div className="bg-[#1b1c20] p-3 md:p-4 rounded-md relative overflow-visible">
            {top5.map((anime, index) => (
              <Link
                key={index}
                href={`/anime/${anime.slug ?? ''}`}
                className="flex gap-2 md:gap-3 items-start py-2 md:py-[10px] border-b border-zinc-700 min-h-[80px] md:h-[112px] last:border-none group"
              >
                {/* Rank */}
                <div className="flex flex-col items-center w-6 md:w-8 flex-shrink-0 pt-1.5 self-center group">
                  <span
                    className={`font-bold text-xs md:text-sm transition-colors duration-200 ${
                ...
                {/* Thumbnail */}
                <div className="w-[50px] md:w-[65px] h-[65px] md:h-[85px] relative overflow-hidden rounded-md flex-shrink-0">
                  <Image src={anime.image} alt={anime.title} fill sizes="(max-width: 768px) 50px, 65px" className="object-cover" />
                </div>

                {/* Info */}
                <div className="flex-1 text-white text-[11px] md:text-[12px] leading-snug self-center relative overflow-visible">
...
function MovieCardComponent({ movie }: { movie: MovieCard }) {
  return (
    <div>
      <div className="relative h-[160px] md:h-[200px] lg:h-[230px] bg-zinc-800 rounded-md overflow-hidden">
        <Image
          src={movie.image}
          alt={movie.title}
          fill
          sizes="(max-width: 768px) 33vw, (max-width: 1024px) 25vw, 163px"
          className="object-cover transition duration-300 group-hover:blur-xs"
        />
        <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <FaPlay className="text-white text-xl md:text-2xl lg:text-3xl" />
        </div>
      </div>
