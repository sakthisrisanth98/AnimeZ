    ...
    <main className="min-h-screen m-0 p-0 bg-black text-white font-sans" style={{ backgroundImage: 'url(/black-paper.png)', backgroundRepeat: 'repeat', backgroundPosition: 'center' }}>
      ...
      <header className="flex justify-between items-center px-4 md:px-6 bg-[#111] h-[72px] relative z-30">
        <div className="flex items-center gap-2 md:gap-6">
          ...
          <Image src="/animez.png" alt="AnimeZ" width={120} height={32} priority className="object-contain md:w-[170px] md:h-[46px]" />
        </div>
        ...
        {menuOpen && (
          <>
            ...
            />
            <aside className={`fixed top-0 left-0 z-50 h-full max-h-screen w-[280px] md:w-[325px] bg-[#121212] text-white flex flex-col transition-transform duration-300 shadow-lg border-r border-zinc-800 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <section className="px-4 md:px-6 pt-10 pb-0">
        ...
        <div className="flex items-center mb-4 md:mb-6">
          <h3 className="text-xl md:text-2xl font-bold text-pink-400">All Animes</h3>
        </div>
        
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {/* Anime Grid - Mobile Responsive */}
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-6 flex-1 self-start">
            {currentAnime.map((anime) => (
              ...
              >
                <div className="bg-zinc-800 rounded-md overflow-hidden h-[160px] md:h-[210px] relative">
                  <Image
                    src={anime.image}
                    alt={anime.title}
                    fill
                    sizes="(max-width: 768px) 33vw, (max-width: 1024px) 25vw, 140px"
                    className="object-cover transition duration-300 group-hover:blur-sm"
                  />
                  {/* Centered play icon */}
                  <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <FaPlay className="text-white text-xl md:text-3xl" />
                  </div>
                </div>
                <div className="mt-2 px-1">
                  <h4
                    className="text-white hover:text-pink-400 font-semibold text-sm md:text-base truncate transition-colors duration-150"
                    title={anime.title}
                  >
                    {anime.title}
                  </h4>
          ...
          {/* Sidebar Top 10 - Mobile Responsive */}
              ...
              {getActiveList().map((anime, index) => (
                ...
                >
                  <div className="flex flex-col items-center w-6 flex-shrink-0 self-center group">
                    ...
                    {index < 3 && (
                      <span className="w-[16px] md:w-[20px] h-[2px] md:h-[3px] bg-pink-400 mt-[2px] rounded-full" />
                    )}
                  </div>
